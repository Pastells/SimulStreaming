...
class LLMTranslator:
    def __init__(
        self,
        system_prompt="Please translate.",
        max_context_length=4096,
        len_ratio=None,
        model_dir="ct2_EuroLLM-9B-Instruct/",
        tokenizer_dir="EuroLLM-9B-Instruct/",
    ):
    ...
    def build_prompt(self, dialog):
        ...
        base_toks = self.tokenizer.apply_chat_template(
            dialog[:2], tokenize=True, add_generation_prompt=True
        )["input_ids"]
...
class ParallelTextBuffer:
    ...
    def trim_segments(self):
        ...
        for s, t in zip(self.src_buffer, self.tgt_buffer):
            print(
                "\t", s, "...", t, file=sys.stderr
            )  # ,self.src_buffer, self.tgt_buffer, file=sys.stderr)
        src = " ".join(" ".join(b) for b in self.src_buffer)
        tgt = "".join(self.tgt_buffer)
...
class SimulLLM:
    ...
    def init(self):
        self.buffer = ParallelTextBuffer(
            self.llmtranslator.tokenizer,
            self.llmtranslator.max_tokens_to_trim,
            trimming=self.trimming,
            init_src=self.init_src,
            init_tgt=self.init_tgt,
        )
...
def translate_args(parser):
    ...
    parser.add_argument(
        "--model-dir",
        type=str,
        default="ct2_EuroLLM-9B-Instruct/",
        help="ct2 model directory. If not set, the default ct2_EuroLLM-9B-Instruct/ is used.",
    )
    parser.add_argument(
        "--tokenizer-dir",
        type=str,
        default="EuroLLM-9B-Instruct/",
        help="tokenizer directory. If not set, the default EuroLLM-9B-Instruct/ is used.",
    )
...
def simulation_args(parser):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Filename of jsonl file to simulate input. If not set, it is read from stdin.",
    )
...
def simul_translator_factory(args):
    ...
    if args.init_prompt_src is None:
        ...
        if args.init_prompt_tgt is None:
            ...
            if args.tgt_lan == "ja" and args.src_lan == "en":
                ...
                print(
                    "WARNING: Default init_prompt_src not set, src_lan is English and tgt_lan is Japanese. The init_src prompt changed to be more verbose.",
                    file=sys.stderr,
                )
        else:
            print(
                "WARNING: init_prompt_tgt is used, init_prompt_src is None, the default one. It may be wrong!",
                file=sys.stderr,
            )
        ...
        if args.init_prompt_tgt is None:
            print(
                "WARNING: init_prompt_src is used, init_prompt_tgt is None, so the default one is used. It may be wrong!",
                file=sys.stderr,
            )
    ...
    if args.language_specific_len_threshold:
        if args.len_threshold is not None:
            print(
                "ERROR: --len-threshold is set, but --language-specific-len-threshold is also set. Only one can be used.",
                file=sys.stderr,
            )
            sys.exit(1)
        else:
    ...
    llmtrans = LLMTranslator(
        system_prompt=sys_prompt,
        max_context_length=args.max_context_length,
        len_ratio=len_threshold,
...
def simulation_update(simul, rows, timer, out_handler=handle_outputs):
    ...
    if inserted:
        out = simul.process_iter(
            is_final=False
        )  # if the last row is final, it was already processed.
        out_handler(out, row, timer)
...
def main_simulation_from_file():
    ...
    if not args.comp_unaware and args.input_jsonl is None:
        print(
            f"ERROR: Reading stdin in computationally aware simulation does not work. Loading the model breaks timing. Use --input-jsonl instead.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(
        f"INFO: Reading {inform_ifile} in jsonl format, computationally {un}aware simulation.",
        file=sys.stderr,
    )
