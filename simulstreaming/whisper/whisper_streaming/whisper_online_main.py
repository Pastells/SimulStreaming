...
def processor_args(parser):
    ...
    parser.add_argument(
        "--out-txt",
        action="store_true",
        help="Output formatted as not as jsonl but simple space-separated text: beg, end, text",
        default=False,
    )
...
def simulation_args(parser):
    ...
    simulation_group.add_argument(
        "--start_at", type=float, default=0.0, help="Start processing audio at this time."
    )
